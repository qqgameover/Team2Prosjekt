const model = {
    ...
    data: {
        ...
        statistikk: {
            ...
            meldinger: [],
            instanser: [
                { id: 0, lagnavn: "Team-1", points: 0 },
                { id: 1, lagnavn: "Team-2", points: 0 },
                { id: 2, lagnavn: "Team-3", points: 0 },
                { id: 3, lagnavn: "Team-4", points: 0 },
                { id: 4, lagnavn: "Team-5", points: 0 },
                { id: 5, lagnavn: "Team-6", points: 0 },
                { id: 6, lagnavn: "Team-7", points: 0 },
                { id: 7, lagnavn: "Team-8", points: 0 },
            ],
            achievements: [],
        },
        muligeMeldinger: [
...
function onSignIn(googleUser) {
    ...
    model.app.currentPage = 'teamSelector'
    getData();
}
...
async function initClient() {
    ...
    try {
        ...
        console.error(error, "DANGER DANGER")
    }
}
...
function addAch(_user, _kategori, _points, _motakker = "") {
    var point = {
        lagnavn: model.app.selectedTeam,
        points: _points,
    };
    ...
    await pointsCollection.get().then(
        (pointCollectionn) => {
            ...
            pointCollectionn.forEach(function (pointsColl) {
                let points = pointsColl.data();
                console.log("points");
                model.data.statistikk.achievements.push({lagnavn: points.lagnavn, points: points.points});
            }); addPoints();
        });
